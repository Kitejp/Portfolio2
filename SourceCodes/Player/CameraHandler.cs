...
namespace KH
{
    public class CameraHandler : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // 「～」: ビットの値を反転、「<<」:左側のオペランドが、右側のオペランドで定義されたビット数だけ左にシフトする。
        ...
        public void HandleLockOn()
        {
            ...
            for (int i = 0; i < colliders.Length; i++)
            {
                ...
                if (character != null)
                {
                    Vector3 lockOnTargetDistance = character.transform.position - targetTransform.position;
                    float distanceFromTarget = Vector3.Distance(targetTransform.position, character.transform.position);
                    float viewableAngle = Vector3.Angle(lockOnTargetDistance, cameraTransform.forward);
                    RaycastHit hit;
                        ...
                        && viewableAngle > -50 && viewableAngle < 50 && distanceFromTarget <= maximumLockOnDistance)
                    {
                        if (Physics.Linecast(playerManager.lockOnTransform.position, character.lockOnTransform.position, out hit))
                        {
                            ...
                            {
                                //プレイヤーと敵との間に障害物がある場合、ロックできないようにする
